AddEventHandler('challenge.OutpostLiberated', function(oID, faction, mvp, others) --keep in mind that we only use 
    local challenges = {}
    local _faction = factionIdToName[tonumber(faction)]
    ...
    for i,k in pairs(sharedChallenges) do
        if k.status == "active" then
            if k.missionType == "outpost" and k.missionCondition1 == "liberate" then
                if k.missionCondition2 == "any" or k.missionCondition2 == _faction[1] then
...
AddEventHandler('challenge.OutpostDestroyed', function(oID, faction, mvp, others) --keep in mind that we only use 
    local challenges = {}
    local _faction = factionIdToName[tonumber(faction)]
    local src = tonumber(mvp)
    for i,k in pairs(sharedChallenges) do
        if k.status == "active" then
            if k.missionType == "outpost" and k.missionCondition1 == "destroy" then
                if k.missionCondition2 == "any" or k.missionCondition2 == _faction[1] then
                    table.insert(challenges, i)
                end
            end
